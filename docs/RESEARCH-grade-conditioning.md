...
**우리 실측이 이를 뒷받침한다.** 교사(32B)에게 등급 라벨만 준 v1과, "왜 그렇게 판단했는지
설명하라"로 바꾼 v2를 답 있는 사례에서만 비교했다.

| | v1 (라벨만) | v2 (근거 설명 요구) |
|---|---|---|
| 답 있는 사례 | n=214 | n=230 |
| 확정도 반영률 (4마커 탐지기) | 67% | **93%** |
| 확정도 반영률 (5마커 탐지기) | 74% | **98%** |

32B조차 라벨만 보고는 3분의 1을 흘렸다. 1.2B에 프롬프트만으로 넘겼다면 실패했을 설계다.

> 정정: 이전 판에 "66% → 99%"로 적었는데 두 가지가 틀렸다. (1) 거절 사례를 포함해 세서
> v1/v2의 거절 비율 차이(20% vs 28%)가 수치를 교란했다. (2) 두 조건을 **다른 탐지기로**
> 쟀다 — v1은 4마커, v2는 `조건`을 더한 5마커였고 그 한 마커가 v2에 유리하게 5~7%p를
> 얹는다. 위 표는 조건마다 두 탐지기를 모두 적용한 것이다. 어느 쪽으로 재도 26%p 이상
> 벌어지므로 결론은 유지되지만, 발표할 수치는 탐지기를 맞춘 쪽이어야 한다.
...
### 그 15%는 오류가 아니라 **버그의 증상**이었다 (2026-07-28, 독립 감사)

위 기록을 "내 보고 실수"로 정리했는데, 감사에서 더 깊은 원인이 나왔다. 챗 학습 데이터가
등급을 DB의 `weight` 값에서 **역산**하고 있었다:

```
weight = COMMIT_W[commitment] × SAL_W[salience]      # backfill_weights.py
grade(w) = 확정 if w≥0.9 · 조건부 if w≥0.75 · 예정 if w≥0.5 · 논의  # build_chat_sft_jobs.py
```

**곱은 되돌릴 수 없다.** 실제 940건으로 재현한 결과:

| 실제 commitment | salience | 모델이 본 등급 | 건수 |
|---|---|---|---|
| 확정 | core | 확정 ✓ | 369 |
| **확정** | **supporting** | **조건부 ✗** | **229** |
| 의지예정 | supporting/core | 예정 ✓ | 269 |
| 의지예정 | peripheral | 논의 ✗ | 15 |
| 확정 | peripheral | 예정 ✗ | 6 |
| 조건부확정 | core | 확정 ✗ | 6 |
| 조건부확정 | supporting | 예정 ✗ | 2 |

258/940 = **27%가 잘못된 등급으로 학습되고 있었다.** 특히 조건부 축이 정확히 뒤집혀 있다:

- 학습 데이터의 `[조건부]` 229건은 **전부 오탐** — 하나도 빠짐없이 실제로는 확정이다.
- 진짜 조건부확정 8건은 **하나도** `[조건부]`로 가지 않았다(6건 확정, 2건 예정).
- 즉 정밀도 0%, 재현율 0%. 모델은 "확정인데 부수적인 결정"에 조건을 붙여 말하도록
  학습되고 있었다.

salience(위상)가 commitment(확정도) 채널로 새어 들어간 범주 오류다. §3에서 조건부를
축에서 분리해야 한다고 논증했는데, 코드는 **다른 축을 조건부 칸에 밀어넣고 있었다.**

수정: `weight`에서 역산하지 않고 판정 결과의 `commitment`를 직접 읽는다. `weight`는
설계 문서가 원래 말한 대로 검색 랭킹 정렬용으로만 쓴다. 이 수정으로 `[조건부]` 노출은
229건 → 8건(0.85%)이 되며, 이는 v2 판정이 독립적으로 낸 조건부 비율 1.0%와 일치한다.
**서로 다른 두 방법이 같은 수를 낸 것이 이 수정의 검증이다.**

교훈은 §2의 CAG 관찰과 같은 자리에 있다 — 등급 라벨링 정확도가 성능 상한인데, 우리는
판정 모델이 아니라 **라벨을 저장하고 꺼내는 배관**에서 정확도를 잃고 있었다.
