...
### 실측 — 축 분리의 효과는 방향은 맞았고 규모는 작았다

같은 924건을 v1(단일 축)과 v2(조건 분리)로 판정해 대조했다. 라벨 이름이 달라
(v1 `의지예정`/`제안검토` vs v2 `예정`/`논의`) 정규화 후 비교했다.

| | 결과 |
|---|---|
| commitment 등급 일치 | 749/924 (81%) |
| v2가 잡은 조건부 | **9건 (1.0%)** |
| 그중 v1이 조건부로 분류한 것 | **2건** — 나머지 7건은 확정·예정·제안검토로 흩어져 조건이 소실 |
| v1 `조건부확정` 8건의 실체 | 4건은 조건이 아닌 단순 예정, 2건만 진짜 조건부 확정 |

**축을 섞으면 양방향으로 틀린다는 것이 확인됐다** — 조건을 놓치기도 하고(7/9), 조건이
아닌 것을 조건 칸에 담기도 한다(4/8). 이론적 근거대로다.

**그러나 이 코퍼스에서 실익은 작다.** 조건부 결정이 전체의 1%뿐이다. 사내 회의록에서
"~하면 ~한다" 형태의 조건부 합의가 드물기 때문으로 보인다. 분리가 옳은 설계지만 이
도메인에서는 효과가 미미했다고 쓰는 것이 정직하다.

**오히려 더 큰 문제가 드러났다.** 예정↔확정 불일치가 145건(15.7%)으로 조건 문제보다
훨씬 크다. 두 판정이 같은 프롬프트·같은 모델·temperature 0인데도 실현성 축에서
이만큼 흔들린다는 것은, **판정 안정성 자체가 조건 축 분리보다 시급한 과제**라는 뜻이다.
등급 라벨링 정확도가 성능 상한이라는 CAG의 관찰(§2)과 겹치는 지점이다.

> 기록: 초기에 "조건부가 15%"라고 보고했으나 이는 오류였다. 챗 프롬프트에서 weight 값을
> 구간으로 환산한 분포와 판정 라벨 분포를 혼동한 것이다. 실제 v1 `조건부확정`은 0.9%였다.
